...
import BusinessLogic.HormigaClasificacionBL;
import DataAccessComponent.DTO.HormigaClasificacionDTO;

public class HormigaClasificacionPanel extends JPanel {
    ...
    public HormigaClasificacionPanel() {
        initializeUI();
        loadTableData();
    }
    ...
    private void initializeUI() {
        ...
        btnCreate.addActionListener(e -> createHormigaClasificacion());
        btnUpdate.addActionListener(e -> updateHormigaClasificacion());
        btnDelete.addActionListener(e -> deleteHormigaClasificacion());
    ...
    private void loadTableData() {
        try {
            ...
            HormigaClasificacionBL HormigaClasificacionBL = new HormigaClasificacionBL();
            List<HormigaClasificacionDTO> Relacions = HormigaClasificacionBL.readAll();
            ...
            for (HormigaClasificacionDTO Relacion : Relacions) {
                model.addRow(new Object[]{
                        Relacion.getIdHormigaClasificacion(),
                        Relacion.getNombre(),
                        Relacion.getEstado(),
    ...
    private int getTotalRecords() {
        try {
            HormigaClasificacionBL HormigaClasificacionBL = new HormigaClasificacionBL();
            return HormigaClasificacionBL.getMaxRow();
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    private void createHormigaClasificacion() {
    ...
    if (nombre != null && !nombre.isEmpty()) {
        try {
            HormigaClasificacionDTO nuevoRelacion = new HormigaClasificacionDTO();
            nuevoRelacion.setNombre(nombre);

            HormigaClasificacionBL HormigaClasificacionBL = new HormigaClasificacionBL();
            boolean creado = HormigaClasificacionBL.create(nuevoRelacion);
    ...
    private void updateHormigaClasificacion() {
    ...
    if (selectedRow != -1) {
        ...
        if (nuevoNombre != null && !nuevoNombre.isEmpty()) {
            try {
                HormigaClasificacionDTO RelacionActualizado = new HormigaClasificacionDTO();
                RelacionActualizado.setIdHormigaClasificacion(RelacionID);
                RelacionActualizado.setNombre(nuevoNombre);

                HormigaClasificacionBL HormigaClasificacionBL = new HormigaClasificacionBL();
                boolean actualizado = HormigaClasificacionBL.update(RelacionActualizado);
    ...
    private void deleteHormigaClasificacion() {
        // Obtener el índice de la fila seleccionada
    int selectedRow = table.getSelectedRow();
    if (selectedRow != -1) {
        ...
        if (option == JOptionPane.YES_OPTION) {
            ...
            try {
                HormigaClasificacionBL HormigaClasificacionBL = new HormigaClasificacionBL();
                boolean eliminado = HormigaClasificacionBL.delete(RelacionID);
