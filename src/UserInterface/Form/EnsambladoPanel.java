...
import BusinessLogic.EnsambladoBL;
import DataAccessComponent.DTO.EnsambladoDTO;

public class EnsambladoPanel extends JPanel {
    ...
    private void initializeUI() {
        ...
        btnCreate.addActionListener(e -> createEnsamblado());
        btnUpdate.addActionListener(e -> updateEnsamblado());
        btnDelete.addActionListener(e -> deleteEnsamblado());
    ...
    private void loadTableData() {
        try {
            ...
            EnsambladoBL EnsambladoBL = new EnsambladoBL();
            List<EnsambladoDTO> Relacions = EnsambladoBL.readAll();
            ...
            for (EnsambladoDTO Relacion : Relacions) {
                model.addRow(new Object[]{
                        Relacion.getIdEnsamblado(),
                        Relacion.getNombre(),
                        Relacion.getEstado(),
    ...
    if (nombre != null && !nombre.isEmpty()) {
        try {
            EnsambladoDTO nuevoRelacion = new EnsambladoDTO();
            nuevoRelacion.setNombre(nombre);

            EnsambladoBL EnsambladoBL = new EnsambladoBL();
            boolean creado = EnsambladoBL.create(nuevoRelacion);
    ...
    if (selectedRow != -1) {
        ...
        if (nuevoNombre != null && !nuevoNombre.isEmpty()) {
            try {
                EnsambladoDTO RelacionActualizado = new EnsambladoDTO();
                RelacionActualizado.setIdEnsamblado(RelacionID);
                RelacionActualizado.setNombre(nuevoNombre);

                EnsambladoBL EnsambladoBL = new EnsambladoBL();
                boolean actualizado = EnsambladoBL.update(RelacionActualizado);
    ...
    if (selectedRow != -1) {
        ...
        if (option == JOptionPane.YES_OPTION) {
            ...
            try {
                EnsambladoBL EnsambladoBL = new EnsambladoBL();
                boolean eliminado = EnsambladoBL.delete(RelacionID);
