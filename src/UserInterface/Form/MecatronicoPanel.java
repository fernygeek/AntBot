...
import BusinessLogic.MecatronicoBL;
import DataAccessComponent.DTO.MecatronicoDTO;

public class MecatronicoPanel extends JPanel {
    ...
    private void initializeUI() {
        ...
        btnCreate.addActionListener(e -> createMecatronico());
        btnUpdate.addActionListener(e -> updateMecatronico());
        btnDelete.addActionListener(e -> deleteMecatronico());
    ...
    private void loadTableData() {
        try {
            ...
            MecatronicoBL MecatronicoBL = new MecatronicoBL();
            List<MecatronicoDTO> Relacions = MecatronicoBL.readAll();
            ...
            for (MecatronicoDTO Relacion : Relacions) {
                model.addRow(new Object[]{
                        Relacion.getIdMecatronico(),
                        Relacion.getNombre(),
                        Relacion.getEstado(),
    ...
    private int getTotalRecords() {
        try {
            MecatronicoBL MecatronicoBL = new MecatronicoBL();
            return MecatronicoBL.getMaxRow();
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    private void createMecatronico() {
    ...
    if (nombre != null && !nombre.isEmpty()) {
        try {
            MecatronicoDTO nuevoRelacion = new MecatronicoDTO();
            nuevoRelacion.setNombre(nombre);

            MecatronicoBL MecatronicoBL = new MecatronicoBL();
            boolean creado = MecatronicoBL.create(nuevoRelacion);
    ...
    private void updateMecatronico() {
    ...
    if (selectedRow != -1) {
        ...
        if (nuevoNombre != null && !nuevoNombre.isEmpty()) {
            try {
                MecatronicoDTO RelacionActualizado = new MecatronicoDTO();
                RelacionActualizado.setIdMecatronico(RelacionID);
                RelacionActualizado.setNombre(nuevoNombre);

                MecatronicoBL MecatronicoBL = new MecatronicoBL();
                boolean actualizado = MecatronicoBL.update(RelacionActualizado);
    ...
    private void deleteMecatronico() {
        // Obtener el índice de la fila seleccionada
    int selectedRow = table.getSelectedRow();
    if (selectedRow != -1) {
        ...
        if (option == JOptionPane.YES_OPTION) {
            ...
            try {
                MecatronicoBL MecatronicoBL = new MecatronicoBL();
                boolean eliminado = MecatronicoBL.delete(RelacionID);
